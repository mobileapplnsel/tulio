...
class MotorExport implements FromCollection,WithHeadings,WithEvents,WithTitle
{
    /**
    ...
    */
    public function collection()
    {
        $data = collect();
        $i=1;
        $rooms = $this->project->rooms()->whereHas('products',function($q){
            $q->whereNull('product_id');
        });
        foreach($rooms as $room){
            $data->push([$i++,$room->name]);
            foreach($room->products->whereNull('product_id') as $product){
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                $event->sheet->getStyle("B3:B$i")->getAlignment()->setWrapText(true);
            },
        ];
    }
}
